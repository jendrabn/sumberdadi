                ...
                <div class="col-md-6">
                  <address>
                    ...
                    @if ($payment->method === 'bank')
                      Nomor Rekening: {{ $payment->bank->account_number }} - Bank:
                      {{ strtoupper($payment->bank->bank_code) }}
                    @else
                      Nomor Handphone: {{ $payment->ewallet->phone_number }} - e-Wallet:
                      {{ strtoupper($payment->ewallet->wallet_type) }}
                    @endif
                  </address>
                </div>
                <div class="col-md-6 text-md-right">
            ...
            <div class="col-md-12">
              ...
              <div class="row mt-4">
                <div class="col-lg-8">
                  ...
                  <p class="section-lead">Pastikan Anda telah menerima pembayaran sejumlah yang telah dibebankan sebelum
                    menekan tombol Konfirmasi Pembayaran.</p>
        ...
        <div class="text-md-right">
          <div class="float-lg-left mb-lg-0 mb-3">
            <button class="btn btn-primary btn-icon icon-left"
              id="confirmPayment"
              @if ($payment->status === 'RELEASED' || $payment->status === 'CANCELLED') disabled @endif><i class="fas fa-check"></i> Konfirmasi
              Pembayaran</button>
            <button class="btn btn-danger btn-icon icon-left"
              id="cancelPayment"
              @if ($payment->status !== 'PENDING') disabled @endif><i class="fas fa-times"></i> Batalkan Pesanan</button>
          </div>
          <button class="btn btn-warning btn-icon icon-left"
            onclick="printJS('invoiceRoot', 'html')"><i class="fas fa-print"></i> Print</button>
        </div>
      </div>
    </div>
  </section>
        ...
        Swal.fire({
          ...
          if (result.value) {
            $.ajax({
              ...
              if (data.status) {
                Swal.fire('Sukses!', 'Pesanan sudah terkonfirmasi dan akan diteruskan ke penjual',
                  'success');
                setTimeout(() => {
